...
function isFull() -- checks if there is a free inventory space
  local ret = true
  for i = 0, 14 do
    if turtle.getItemCount(15 - i) == 0 then
      ret = false
      break
    end
  end
...
function diggydiggyhole() -- runs the other functions in the proper order to mine out the user defined area, returns once complete
  trashlist()
  refuel()
  if turtle.getFuelLevel() < 500 then
